...
- Follow existing convention: `<name>.ts`, `<name>.html`, `<name>.css`, `<name>.spec.ts` (no `.component` suffix).
- Use external templates/styles with relative paths (per `angular-standards.md`).
